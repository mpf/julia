...
[Vagrant](http://www.vagrantup.com/downloads) can also be used with a Windows guest VM via the following `Vagrantfile`:
```
# Vagrantfile for building Windows Julia via MSYS2
...
Run `vagrant up` from the same folder as the above `Vagrantfile`. Or, to build with Cygwin, replace the `SCRIPT` section with
```
# change the following to 32 for 32 bit Julia
$bits = "64"
$arch = "x86_$bits".Replace("x86_32", "i686")
...
& "C:\\cygwin$bits\\bin\\sh" -lc "if ! [ -e julia$bits ]; then git clone \\
  ...
  make -j2 test && make win-extras binary-dist"
```
